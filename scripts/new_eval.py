# https://github.com/lartpang/PySODMetrics/blob/main/py_sod_metrics/sod_metrics.py
...
# Metric classes as defined in the provided code (IoU, BIoU, TIoU, Fmeasure, Mae, Smeasure, Emeasure, WeightedFmeasure, BoundaryAccuracy)

# Assuming all classes (IoU, BIoU, TIoU, Fmeasure, Mae, Smeasure, Emeasure, WeightedFmeasure, BoundaryAccuracy) 
# are defined here exactly as they were in the provided code.

def _prepare_data(pred: np.ndarray, gt: np.ndarray) -> tuple:
    # assert len(np.unique(pred)) == 2 and (np.max(pred) == 1 or np.min(pred) == 0)
    assert ((pred != 0)&(pred != 1)).sum() == 0
    gt = gt > 128
    # pred = pred / 255
    pred = pred / 1
...
class Dice(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        dices = self.cal_dice(pred=pred, gt=gt)
        self.dices.append(dices)

    def _prepare_data(self, pred, gt):
        # Ensure the prediction and ground truth are binarized
        pred = (pred > 0.5).astype(np.uint8)
        gt = (gt > 0.5).astype(np.uint8)
        return pred, gt

    def cal_dice(self, pred, gt):
        # Convert to binary if needed, but assuming pred and gt are already binary (0 or 1)
        
        TPs = np.sum((pred == 1) & (gt == 1))  # True Positives
        FPs = np.sum((pred == 1) & (gt == 0))  # False Positives
        FNs = np.sum((pred == 0) & (gt == 1))  # False Negatives
    
        denominator = 2 * TPs + FPs + FNs
        if denominator == 0:
            return 1  # Perfect match if both prediction and ground truth are empty
    
        dice_score = (2 * TPs) / denominator
        return dice_score

    def get_results(self) -> dict:
        dice = np.mean(np.array(self.dices, dtype=np.float32), axis=0)
        # Convert the numpy array or float32 type to a Python list or float
        return dict(dice=dict(curve=dice.tolist()))  # Use tolist() to ensure JSON compatibility
...
def calculate_metrics(predictions, ground_truths):
    ...
    tiou_metric = TIoU()
    dice_metric = Dice()
    fmeasure_metric = Fmeasure()
    mae_metric = Mae()
    smeasure_metric = Smeasure()
    emeasure_metric = Emeasure()
    wfm_metric = WeightedFmeasure()
    boundary_accuracy_metric = BoundaryAccuracy()

    # Compute the metrics
    for pred, gt in zip(predictions, ground_truths):
        iou_metric.step(pred, gt)
        biou_metric.step(pred, gt)
        tiou_metric.step(pred, gt)
        dice_metric.step(pred, gt)
        fmeasure_metric.step(pred, gt)
        mae_metric.step(pred, gt)
        smeasure_metric.step(pred, gt)
        emeasure_metric.step(pred, gt)
        wfm_metric.step(pred, gt)
        boundary_accuracy_metric.step(pred, gt)

    # Get the final results
    results = {
        "IoU Results": iou_metric.get_results(),
        "BIoU Results": biou_metric.get_results(),
        "TIoU Results": tiou_metric.get_results(),
        "Dice Results": dice_metric.get_results(),
        "F-measure Results": fmeasure_metric.get_results(),
        "MAE Results": mae_metric.get_results(),
        "S-measure Results": smeasure_metric.get_results(),
        "E-measure Results": emeasure_metric.get_results(),
        "Weighted F-measure Results": wfm_metric.get_results(),
        "Boundary Accuracy Results": boundary_accuracy_metric.get_results(),
    }
...
def parse_result_json(file_path):
    ...
    for key in results:
        iou_values.append(np.nanmean(results[key]['IoU Results']['iou']['curve']))
        biou_values.append(np.nanmean(results[key]['BIoU Results']['biou']['curve']))
        tiou_values.append(np.nanmean(results[key]['TIoU Results']['tiou']['curve']))
        mdice_values.append(np.nanmean(results[key]['Dice Results']['dice']['curve']))
        fm_values.append(np.nanmean(results[key]['F-measure Results']['fm']['curve']))
        mae_values.append(results[key]['MAE Results']['mae'] if results[key]['MAE Results']['mae'] is not None else np.nan)
        sm_values.append(results[key]['S-measure Results']['sm'] if results[key]['S-measure Results']['sm'] is not None else np.nan)
        em_values.append(np.nanmean(results[key]['E-measure Results']['em']['curve']))
        wfm_values.append(results[key]['Weighted F-measure Results']['wfm'] if results[key]['Weighted F-measure Results']['wfm'] is not None else np.nan)
        ba_values.append(results[key]['Boundary Accuracy Results']['mba'] if results[key]['Boundary Accuracy Results']['mba'] is not None else np.nan)

    mIoU = np.nanmean(iou_values)
    mBIoU = np.nanmean(biou_values)
    mTIoU = np.nanmean(tiou_values)
    mDice = np.nanmean(mdice_values)
    mFmeasure = np.nanmean(fm_values)
    mMAE = np.nanmean(mae_values)
    mSmeasure = np.nanmean(sm_values)
    mEmeasure = np.nanmean(em_values)
    mWeightedFmeasure = np.nanmean(wfm_values)
    mBoundaryAccuracy = np.nanmean(ba_values)

    return {
        'S-measure': mSmeasure,
        'Weighted F-measure': mWeightedFmeasure,
        'MAE': mMAE,
        'F-measure': mFmeasure,
        'E-measure': mEmeasure,
        'Dice': mDice,
        ...
        'BIoU': mBIoU,
        'TIoU': mTIoU,
        'Boundary Accuracy': mBoundaryAccuracy
    }

def convert_ndarray_to_list(data):
    """
    Recursively convert numpy arrays to lists in the given data structure.
    ...
    if isinstance(data, dict):
        ...
        # Convert numpy array to list and handle NaN
        return [None if np.isnan(item) else item for item in data.tolist()]
