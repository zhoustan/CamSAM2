...
def build_decamouflaged_sam2_video_predictor(
    config_file,
    ckpt_path=None,
    device="cuda",
    mode="eval",
    ...
    **kwargs,
    ...
    hydra_overrides = [
        "++model._target_=sam2.decamouflaged_sam2_video_predictor.SAM2VideoPredictor",
    ]
    if apply_postprocessing:
        hydra_overrides_extra = hydra_overrides_extra.copy()
        hydra_overrides_extra += [
...
#         # Correct any key mismatches
#         corrected_sd = {}
#         for key, value in sd.items():
#             corrected_key = key.replace("DeCamouflaged_", "decamouflaged_").replace("embedding_maskfeature", "embedding_mask_feature")
#             corrected_sd[corrected_key] = value

#         # Load the corrected state dict into the model
#         missing_keys, unexpected_keys = model.load_state_dict(corrected_sd, strict=False)
